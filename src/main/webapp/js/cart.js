...
function addCart() {
	...
	// 跳转到后台取出商品
	window.location.href = "http://localhost:8080/eb/cart_add?entityId="
			+entityId+ "&quantity="+quantity;
...
function addCartByParam(entityId, quantity) {
	...
	$.ajax({
		...
		success : function(jsonStr) {
			...
			alert(j);
			window.location.href = "http://localhost:8080/eb/toSettlement.jsp?product="
					+ jsonStr;
...
//	alert(index);
	if(index != 0){
		//有东西   
		window.location.href = "http://localhost:8080/eb/cart_addOrder";//后台判断是否登录
	}else{
		//无东西
		alert("您好，购物车为空，请购物！");
		window.location.href = "http://localhost:8080/eb/easybuy_queryAll";//主页
