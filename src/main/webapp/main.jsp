									...
									<div class="zj_l">
										<c:forEach items="${category.categorys }" var="cate2">
											<div class="zj_l_c">
												<h2>
													<a
														href="goodsList_queryGoodsBylevel?level=2&id=${products2.id }&page=1">${cate2.name }</a>
												</h2>	
												<c:forEach items="${cate2.categorys }" var="cate3">
													<a href="productList?level=3&id=${cate3.id }&page=1">${cate3.name }</a>|
												</c:forEach>
											</div>
										</c:forEach>
									</div>
	...
	<div class="i_bg bg_color">
		...
		<c:forEach items="${categoryVOList }" var="categoryVO">
			...
			<div class="i_t mar_10">
				<span class="floor_num">${categoryVO.name }</span> <span class="fl">${categoryVO.name }</span>
			</div>
			...
			<div class="content">
				<div class="fresh_left">
					...
					<div class="fresh_txt">
						<div class="fresh_txt_c">
							<c:forEach items="${categoryVO.categorys }" var="category">
								<a href="goodsList_queryGoodsBylevel?level=2&id=${products2.id }&page=1">${category.name }</a>
							</c:forEach>
						</div>
					</div>
				</div>
				<div class="fresh_mid">
					<ul>
						<c:forEach items="${categoryVO.products }" var="product">
							<li>
									<div class="name">
